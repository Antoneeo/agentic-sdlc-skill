...
- **Ceremony budget declared, per the Vision's `no ceremony ratchet` Non-Goal**: this adds cost at L3 and removes nothing, so it takes that rule's second branch — cost stated, owner accepts explicitly. Cost = one pass before the Impact, one ANALYSIS section, one support file read only when the trigger fires. Accepted by Antonio Pinto, 2026-07-28, scoped to L3; L1 and L2 are untouched. No validator rule lands on consumer projects — a warning on every existing ANALYSIS is the nagging the Vision forbids. Governed by `ai_docs/solutions/ANALYSIS_architect_pass.md`.
