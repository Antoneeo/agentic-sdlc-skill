...
`SKILL.md` -- where the duty is written -- had not been read. `mkt_check.py remind` prints
...
Cost: 437 characters, ~74 tokens per prompt, paid on every turn including
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "python \"C:\Users\<user>\.claude\skills\mkt-agentic-sdlc\scripts\mkt_check.py\" remind"
          }
        ]
      }
    ]
...
`mkt_check.py benefit` reports what the review gates caught and what the
