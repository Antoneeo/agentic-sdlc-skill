...
| F-046 Per-turn protocol reminder (the protocol reaches every turn, not only the first) | L3 | main | DONE, AWAITING PUBLISH | 2026-09-06 | publish code 1.32.0 + kb 1.14.0 + mkt 0.10.0 with publish_all.bat (owner's act, 2FA), then verify on the registry | HANDOFF_per_turn_protocol_reminder.md · this file (the unit record); rulings ledger row r19 |
