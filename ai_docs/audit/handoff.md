...
| F-022 Multi-Domain Core | L3 | feat/multi-domain-core | REVIEW | 2026-07-31 | P0–P7 done and **consolidated into this one repository**: the code distribution at the root, kb and mkt grafted with `git subtree` under `distributions/` (history preserved), their duplicated governance removed on arrival. Drift guard now compares the three copies TO EACH OTHER, not only to a manifest — it caught a one-distribution edit on its first run. All three: batteries green, golden transcripts intact, `npm pack` correct. **Owner's call:** (a) `npm publish` ×3 (2FA; 1.19.0 was already pending, npm is at 1.17.0), (b) archive the old GitHub repos and delete the now-redundant local clones `kb-agentic-skill/` and `mkt_agentic_sdlc/`, (c) whether to publish kb while its knowledge-method overlays are still stubs | ANALYSIS_multi_domain_core.md |
