...
| F-022 Multi-Domain Core | L3 | feat/multi-domain-core | IN_PROGRESS | 2026-07-31 | P0–P2 done: contract shipped, and the validator now splits into `sdlc_core.py` (spine, all three domains' rules) + a thin `sdlc_check.py` code entry point. TS1's golden corpus was captured pre-change (`25a2367`) and stayed byte-identical through the split; 102 python + 12 node green. Next is P2b (docs root as a parameter) — it owes its own impact map before any edit | ANALYSIS_multi_domain_core.md (Diary; no volatile state) |
