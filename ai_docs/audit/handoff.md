...
| F-024 kb Knowledge Method | L3 | feat/kb-knowledge-method | IN_PROGRESS | 2026-08-01 | **the release gate — F-022 stays HELD until this is effective.** IMPLEMENTED: overlay inside kb's `sdlc_check.py` (graph/corpus/claim-id + intercepted index/validate/check, forward-by-default), three method files rewritten with the real method, templates/SKILL/ENFORCEMENT wired. Three design-gate rounds (all FAIL, ~50 findings disposed) + owner rulings: detect-and-hold, `basis:` on rulings, L1 free, batched legal-form escalations. All 3 distribution batteries green, drift guard clean, golden +9/−0 with frozen snapshot, end-to-end smoke OK. **Acceptance run DONE on the Eclosion corpus** (`kb_field_test_eclosion/`: 5 specs, 7 topics, 26 claims, check CLEAN; UC2 355MB/month matches the declared total; UC3 surfaced a real GIVEN-vs-GIVEN capacity conflict, held CONTESTED, escalated in the legal form). Remaining: closure (Component Map, ADR) + the owner's word on effectiveness | ANALYSIS_kb_knowledge_method.md (Diary) |
| F-025 Claim Ledger | L3 | feat/kb-knowledge-method | IN_PROGRESS | 2026-08-01 | IMPLEMENTED inside kb's entry point (two-file validator): parse/id/scope/qty pure functions + check_claims (symmetry, global dup-id, span checks, basis/derived_from refusal), claim-id --fill confined to id cells. 22-test battery green. Remaining: closes with F-024's closure | ANALYSIS_claim_ledger.md (Diary) |
