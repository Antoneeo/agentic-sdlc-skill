...
verified by rebuild-and-diff in the overlay's `validate`. **No database, persisted or
otherwise.** A persisted sqlite artifact is rejected on the ADR's ground — it is the one
derived artifact that cannot be verified by regenerate-and-compare, and a torn one
reports green. The design named `sqlite3.connect(":memory:")` for the check-time graph;
**as implemented it is plain Python structures** — `kb_load_topics` returns a slug→node
dict, `kb_graph_check` resolves tombstones and walks ancestors over it, and `sdlc_check.py`
imports no `sqlite3`. The design's own reason for the in-memory choice (nothing survives
the run, so nothing needs verifying) holds either way, and dicts are the smaller
dependency on a corpus that fits in memory by construction: the graph is rebuilt from the
markdown files on every run. Recorded here because the design said one thing and the code
does another — the substitution is sound, its silence was not.
...
**2026-08-02 — design/implementation drift found by the owner's question, corrected in
the documents.** The design specified `sqlite3.connect(":memory:")` as the check-time
graph; the shipped `sdlc_check.py` imports no `sqlite3` and builds the graph from plain
Python dicts. Behaviour is unaffected (cycles, reachability, double owner and
near-duplicate are covered by `test_kb_graph.py`) and the substitution is the smaller
dependency, so the code stands and §Index and checks + the ADR now say what shipped.
The finding itself is the signal: this is the closure review's job (`review.md` moment 2 —
the diff against the approved design) and on F-024 it did not catch it.
